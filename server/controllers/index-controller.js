...
export const getItem = async (req, res) => {
  const { id } = req.params;
  const url = `https://api.edamam.com/api/recipes/v2/${id}?type=public&beta=true&app_id=${APP_ID}&app_key=${APP_KEY}&field=uri&field=label&field=image&field=healthLabels`;
  console.log(url);
  try {
    ...
    const dataSingle = apiResSingle.data.hits;
